(function ($) {
  ...
  $.widget('aerolab.blockrain', {
    ...
    updateSizes: function () {
      ...
      this._$canvas.attr('width', this._PIXEL_WIDTH).attr('height', this._PIXEL_HEIGHT);
    },
      ...
      if (this._theme.backgroundGrid instanceof Image) {
        ...
        for (var x = 0; x < this._BLOCK_WIDTH; x++) {
          for (var y = 0; y < this._BLOCK_HEIGHT; y++) {
            ...
            this._ctx.drawImage(this._theme.backgroundGrid, 0, 0, this._theme.backgroundGrid.width, this._theme.backgroundGrid.height, cx, cy, this._block_size, this._block_size);
          }
        }
        ...
        var squareDistance = Math.round(this._block_size * 0.3);
      ...
      this._board = {
        ...
        animate: function () {
          ...
          if (!this.paused && !this.gameover) {
            ...
            if (this.dropCount >= this.dropDelay || game.options.autoplay || (this.holding.drop && now - this.holding.drop >= this.holdingThreshold)) {
              drop = true;
              moved = true;
              this.dropCount = 0;
            }
    ...
    _createUI: function () {
      ...
      game._$start = $(
        '<div class="blockrain-start-holder" style="position:absolute;">' +
          ...
          '</div>' +
          '<a class="blockrain-btn blockrain-start-btn">' +
      ...
      game._$gameover = $(
        '<div class="blockrain-game-over-holder" style="position:absolute;">' +
          ...
          '<div class="blockrain-game-over-msg">' +
          this.options.gameOverText +
          '</div>' +
          '<a class="blockrain-btn blockrain-game-over-btn">' +
    ...
    _getNiceShapes: function () {
      ...
      var func = function (filled, checkCollisions, width, height, mode, _one_shape) {
        ...
        var possibles = new Array(width * height),
          ...
          score,
          best_shape,
          best_score = (evil ? 1 : -1) * 999,
          best_orientation,
          best_x,
          best_score_for_shape,
        ...
        for (attr in opts) {
          //TODO - check in random order to prevent later shapes from winning
          shape = opts[attr];
          best_score_for_shape = -999;

          // for each orientation...
          for (i = 0; i < (shape.symmetrical ? 2 : 4); i++) {
            //TODO - only look at unique orientations
    ...
    _setupControls: function (enable) {
        ...
        if (game.options.asdwKeys) {
          switch (evt.keyCode) {
            ...
            case 83:
              ...
              break;
            case 87:
              /*w*/ game._board.cur.rotate('right');
              break;
          }
        }
        switch (evt.keyCode) {
      ...
      if (!game.options.autoplay) {
        if (enable) {
          $(document).bind('keydown.blockrain', keydown).bind('keyup.blockrain', keyup);
        }
      }
    },
